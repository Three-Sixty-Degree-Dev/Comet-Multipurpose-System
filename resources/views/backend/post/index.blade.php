            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-sm btn-primary float-right" href="{{ route('create') }}"><i
                                class="fas fa-plus"> Add New Post</i></a><br>
                        <div style="display: flex; margin-left: 0px; width: 100%;">
                            <a class="badge badge-primary" href="{{ route('index') }}">Published
                                {{ ($published)? $published : '' }}</a>
                            <a style="margin-left: 5px;" class="badge badge-danger"
                                href="{{ route('post.trash') }}">Trash {{ ($trash)? $trash : '' }}</a>
                        </div>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $data->id }}">
                                    ...
                                    <td>
                                        ...
                                        <img width="50"
                                            src="{{ URL::to('/') }}/media/posts/{{ $featured_info -> post_image }}"
                                            alt="">
                                        @elseif($featured_info -> post_gallery != NULL)
                                        <img width="50"
                                            src="{{ URL::to('/') }}/media/posts/{{ $featured_info -> post_gallery[0] }}"
                                            alt="">
                                    ...
                                    <td>
                                        <div
                                            ...
                                            <input type="checkbox" post_id="{{ $data->id }}"
                                                class="custom-control-input post_status"
                                                {{ ($data->status == true)? 'checked="checked"' : '' }}
                                                id="customSwitch_{{ $loop->index+1 }}">
                                            <label class="custom-control-label"
                                                for="customSwitch_{{ $loop->index+1 }}"></label>
                                        </div>
                                    </td>
                                    <td>
                                        <a title="View" id="Post_view" post_view="{{ $data->id }}"
                                            class="btn btn-sm btn-info" href="#" data-toggle="modal"><i
                                                class="fa fa-eye"></i></a>
                                        <a title="Edit" href="{{ route('post.edit', $data->id) }}" id="♂"
                                            class="btn btn-sm btn-warning"><i class="fas fa-edit text-white"></i></a>
                                        <a title="Trash" class="btn btn-sm btn-danger"
                                            href="{{route('post.trash.update', $data->id)}}"><i
